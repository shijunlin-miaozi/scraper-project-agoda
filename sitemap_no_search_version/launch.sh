...
# Command: `./launch.sh --setup` (note: it only installs dependencies, it will not continue to run spider)
if [ "$SETUP" = true ]; then
...
if [ "$RUN_ALL" = true ]; then
    ...
    # Compute total batches based on CSV line count and batch size
    TOTAL_ROWS=$(($(wc -l < "$HOTELS_FILE") - 1))  # subtract header line
    BATCH_COUNT=$(( (TOTAL_ROWS + BATCH_SIZE - 1) / BATCH_SIZE ))  # ceil division

    echo "📊 Found $TOTAL_ROWS rows in $HOTELS_FILE"
    echo "🔁 Running $BATCH_COUNT batches (batch size = $BATCH_SIZE)"
